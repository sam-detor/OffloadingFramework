...
4. Declare the offloadedCode, remoteInit, and remoteInitSVC methods in the relevant places like below:
...
6. Add the following flags to your normal compile/link process: -ffunction-sections -Wl,--gc-section
...
9. Run the spinUpLdScript.py script in the Offloading folder as shown below, giving it a path to the elf file of your recently compiled code:
   ...
   *note this script requires a library called "pyelftools", install if not already
   ...
   This will generate the custom linker script needed to create the init binary
